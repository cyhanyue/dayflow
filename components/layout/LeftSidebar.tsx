...
import { LayoutDashboard, Calendar, BarChart2, Settings, LogOut, Repeat, ChevronDown, ChevronRight, Pencil, Trash2 } from 'lucide-react'
...
export default function LeftSidebar() {
  ...
  useEffect(() => {
    async function fetchRecurring() {
      ...
      // Deduplicate: one entry per series (title + recurrenceRule), keep earliest scheduled
      const seen = new Map<string, Task>()
      for (const t of all) {
        const key = `${t.title}|||${t.recurrenceRule}`
        const existing = seen.get(key)
        if (!existing || (t.scheduledDate ?? '') < (existing.scheduledDate ?? '')) {
          seen.set(key, t)
        }
      }
      setRecurringTasks(
        Array.from(seen.values()).sort((a, b) => a.title.localeCompare(b.title))
      )
    }
    fetchRecurring()
  }, [])
              ...
              <div key={t.id}>
                <div className="group flex items-center gap-1 px-2 py-1.5 rounded hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors">
                  ...
                  <button
                    ...
                    className="flex-shrink-0 p-0.5 text-stone-300 dark:text-stone-600 opacity-0 group-hover:opacity-100 hover:text-indigo-500 transition-all"
                  >
                    <Pencil size={11} />
                  </button>
                  <button
                    onClick={() => setDeleteTarget(deleteTarget === t.id ? null : t.id)}
                    className="flex-shrink-0 p-0.5 text-stone-300 dark:text-stone-600 opacity-0 group-hover:opacity-100 hover:text-red-500 transition-all"
                  >
                    <Trash2 size={11} />
                  </button>
                </div>
                ...
                {deleteTarget === t.id && (
                  <div className="mx-2 mb-1 p-2 bg-stone-50 dark:bg-stone-800 rounded border border-stone-200 dark:border-stone-700 text-xs space-y-0.5">
                    <button
                      onClick={() => handleDelete(t, 'one')}
                      className="w-full text-left px-1.5 py-1 rounded hover:bg-stone-200 dark:hover:bg-stone-700 text-stone-600 dark:text-stone-400 transition-colors"
                    >
                      Delete this only
                    </button>
                    <button
                      onClick={() => handleDelete(t, 'future')}
                      className="w-full text-left px-1.5 py-1 rounded hover:bg-stone-200 dark:hover:bg-stone-700 text-stone-600 dark:text-stone-400 transition-colors"
                    >
                      Delete this &amp; future
                    </button>
                    <button
                      onClick={() => handleDelete(t, 'series')}
                      className="w-full text-left px-1.5 py-1 rounded hover:bg-red-50 dark:hover:bg-red-950/30 text-red-600 dark:text-red-400 transition-colors"
                    >
                      Delete entire series
                    </button>
                    <button
                      onClick={() => setDeleteTarget(null)}
                      className="w-full text-left px-1.5 py-1 text-stone-400 hover:text-stone-600 transition-colors"
                    >
                      Cancel
                    </button>
                  </div>
